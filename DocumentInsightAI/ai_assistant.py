...
class AIAssistant:
    ...
    def answer_question(self, question: str, document_content: str,
                        conversation_history: List[Tuple]) -> Tuple[str, str]:
    ...
    def generate_challenge_questions(self, document_content: str) -> List[str]:
        ...
        try:
            ...
            raise Exception(
                f"Failed to generate challenge questions: {str(e)}")
    ...
    def _build_conversation_context(self,
        ...
        for i, (q, a, j) in enumerate(
                conversation_history[-3:]):  # Last 3 for context
            context += f"Q{i+1}: {q}\nA{i+1}: {a}\n\n"
    ...
    def _parse_answer_response(self, response: str) -> Tuple[str, str]:
        ...
        try:
            ...
            if len(answer_match) > 1:
                answer_part = answer_match[1].split(
                    "JUSTIFICATION:")[0].strip()
                justification_part = response.split(
                ...
                if len(parts) >= 2:
                    ...
                    ), "Based on the document content provided."
        except Exception:
